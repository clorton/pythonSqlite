...
# clorton        fmatch = re.compile(r"[+-]?((\d+(\.\d*)?)|\.\d+)([eE][+-]?[0-9]+)?").match
...
# clorton                self.file_contents.append( {k:(float(v) if re.match(r"[+-]?((\d+(\.\d*)?)|\.\d+)([eE][+-]?[0-9]+)?", v) else v) for (k,v) in row.iteritems() } )
# clorton                self.file_contents.append( {k:(float(v) if fmatch(v) else v) for (k,v) in row.iteritems() } )
                self.file_contents.append( {k:(convert(v)) for (k,v) in row.iteritems() } )
                rows += 1
...
# clorton                    out = [ entry['Lambda_Map'](x) for x in chunk if x['NodeId'] in nodeIds ]
